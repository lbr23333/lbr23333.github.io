...
5. 不同书上对于嵌入的定义也不尽相同,这里进行一个统一:
   ...
   * $\varphi $是同胚,那么称$\varphi $是一个正则嵌入(regular embedding)
    ...
    * [环面上的无理流](https://en.wikipedia.org/wiki/Linear_flow_on_the_torus)
    ...
    称满足上面(c)条件的嵌入子流形为**拟正则嵌入子流形**.

    (d)由于正则嵌入子流形有子空间拓扑,所以由局部欧可知是局部闭的.
...
1. $G=(\R,+) $:$\varphi:G\rightarrow G $是群同态,即$\varphi(x+y)=\varphi(x)+\varphi(y) $.若$\varphi $是连续的,那么可知$\varphi(x)=cx $.
2. $i_a:G\rightarrow G,i_a(x)=axa^{-1} $是Lie群同态,因为
$$
...
这是群结构所带来的,由Lie群的定义可知,左平移与右平移都是光滑的,而且其逆也是光滑的.即左右平移都是微分同胚.实际上,两者的作用一致,在后面通常默认选择左平移处理问题.
...
但实际上马上将看到, $H $ 为Lie群的条件是有点过强的.

> 有李子群,那有没有菠萝群

{{% mathbox type="blue" title="Lie子群的条件" %}}
设$G$是Lie群,$H $是$G $的子群(仅代数意义上的子群).
* 若$H $是拟正则嵌入子流形,那么$H $是Lie子群;
* 若$H $是正规嵌入子流形,那么$H $是闭的Lie子群.
{{% /mathbox %}}

> 拟正则嵌入流形原本的拓扑可能并不与嵌入空间的子空间拓扑一致,例如环面上的无理流.也就是说在嵌入空间中可能并没有一个好的局部性质,但是在正则嵌入流形本身中是好的,由于Lie子群也只是子流形内部封闭的结构,所以并不要求在嵌入空间中有一个好的性质.所以上面拟正则嵌入子流形是Lie子群并不奇怪.
...
但是一般考虑的 $\mathbb{k} $ 都是 $\R,\mathbb{C} $ ,所以并没有太大的问题.

前面的 $\mathbb{k} $ 代数同态具体到Lie代数就是,在 $(\mathfrak{g}_1,[,]_1) $ 与 $(\mathfrak{g}_2,[,]_2) $ 之间有
$$
  \begin{align*}
    &\varphi:\mathfrak{g}_1\rightarrow \mathfrak{g}_2
    \\
...
{{% mathbox type="slate" title="Q:给定 $C^k_{ij} $ 能否找到所有的Lie代数结构? 设 $\mathbb{k}=\mathbb{C} $ , $V $ 是一个 $n $ 维的 $\mathbb{C} $ 空间,那么就是在问 $V $ 上有多少Lie括号?" %}}
...
$$
很容易验证$[,]_{\mathfrak{g}_{\mathbb{K}}} $:
1.  反称
2.  满足 Jacobi 恒等式(依赖于 $\mathbb{K} $ 是交换、结合的)
...
$$
对于不同Lie代数 $g_i $ 中的元素 $x_i $ ,其之间是不相关的.即 $[x_i,x_j]=0,i\ne j $ ,于是
$$
  [g_i,g_j]_g=0
$$
称
...
$$
为 $g_1,\dots,g_m $ 的直和.(在老瓦的书上是用笛卡尔积处理的,结果是一致的,因为有限维的直和与笛卡尔积本来就是一样的.)
...
更具体地,不可分解是表示为分块上三角,而不可约表示为分块对角.
